...
- **Decision:** `0` (and non-numeric or empty input) is refused on every quantity field by `^[1-9][0-9]*$`, and the prompt names its unit. `0` is deliberately **not** a sentinel for "unlimited" or "no limit"; an operator who wants a practically unbounded value enters a very large number instead. Because the rule is identical on every field, it is stated **once per screen** - a single `0 not allowed` notice immediately before the first numeric field - rather than repeated in each prompt; the prompt itself carries only what varies, the measurement. The validation loops and their red error are what actually enforce the decision; the notice is its description.
