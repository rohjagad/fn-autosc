...
userlock=$(ls /var/log/create/xray/split/ | grep '.locked$' | sed 's/\.locked$//')

# Variabel untuk mencatat file yang dihapus
deleted_users=""

# Cek apakah ada file log yang sesuai dengan pola /var/log/create/xray/split/*.log
log_files=$(ls /var/log/create/xray/split/*.log 2>/dev/null)

if [ -z "$log_files" ]; then
    echo "Tidak ada file log yang ditemukan untuk diproses."
else
    # Perulangan untuk setiap file pengguna di /var/log/create/xray/split/
    for file in /var/log/create/xray/split/*.log; do
