...
for user in "${usernames[@]}"; do
    uid=$(grep "${user}" /etc/xray/json/upgrade.json | awk -F'"id": "' '{print $2}' | awk -F'"' '{print $1}' | sort | uniq)
...
# GET OLD UUID
old=$(grep "${user}" /etc/xray/json/upgrade.json | awk -F'"id": "' '{print $2}' | awk -F'"' '{print $1}' | sort | uniq)
