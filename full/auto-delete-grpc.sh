...
# Cek apakah ada file log yang sesuai dengan pola /var/log/create/xray/grpc/*.log
log_files=$(ls /var/log/create/xray/grpc/*.log 2>/dev/null)

if [ -z "$log_files" ]; then
    echo "Tidak ada file log yang ditemukan untuk diproses."
else
    # Perulangan untuk setiap file pengguna di /var/log/create/xray/grpc/
