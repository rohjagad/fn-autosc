...
func ParseStats(user string, direction string) int64 {
	output, err := ExecuteCommand("xray", "api", "statsquery", "--server=127.0.0.1:10083")
	if err != nil {
		return 0
	}
...
func main() {
	...
	configPath := "/etc/xray/json/grpc.json"
	if _, err := os.Stat(configPath); os.IsNotExist(err) {
		fmt.Println("Config file /etc/xray/json/grpc.json not found!")
		return
	}
	...
	logPath := "/var/log/xray/grpc.log"
	if _, err := os.Stat(logPath); os.IsNotExist(err) {
		fmt.Println("Log file /var/log/xray/grpc.log not found!")
		return
	}

	// Process each user
	for _, user := range users {
		ipCountOutput, err := ExecuteCommand("xray", "api", "statsonline", "--server=127.0.0.1:10083", "-email", user)
		...
		quotaUsage := ReadFile(filepath.Join("/etc/xray/quota/grpc", user+"_usage"))
		quotaLimit := ReadFile(filepath.Join("/etc/xray/quota/grpc", user))
		var quota string
		...
		// IP limit
		ipLimit := ReadFile(filepath.Join("/etc/xray/limit/ip/xray/grpc", user))
		if ipLimit == "" {
			ipLimit = "Not available"
		}

		// Protocol
		protocolLogPath := fmt.Sprintf("/var/log/create/xray/grpc/%s.log", user)
