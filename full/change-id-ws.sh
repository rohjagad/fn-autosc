...
while true; do
    ...
    if [[ -z "$user" || ! -f "/var/log/create/xray/ws/${user}.log" ]]; then
...
sed -i -E "s|^( *UUID[[:space:]]*:).*|\1 ${new}|" /var/log/create/xray/ws/${user}.log
[ -n "$old" ] && sed -i "s|${old}|${new}|g" /var/log/create/xray/ws/${user}.log
...
python3 - "$old" "$new" "/var/log/create/xray/ws/${user}.log" <<'PYEOF' 2>/dev/null || true
