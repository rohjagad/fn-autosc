...
func main() {
	...
	configPath := "/etc/xray/json/upgrade.json"
	if _, err := os.Stat(configPath); os.IsNotExist(err) {
		fmt.Println("Config file /etc/xray/json/upgrade.json not found!")
		return
	}
	...
	logPath := "/var/log/xray/upgrade.log"
	if _, err := os.Stat(logPath); os.IsNotExist(err) {
		fmt.Println("Log file /var/log/xray/upgrade.log not found!")
		return
	}
	...
	for _, user := range users {
		...
		quotaUsage := ReadFile(filepath.Join("/etc/xray/quota/http", user+"_usage"))
		quotaLimit := ReadFile(filepath.Join("/etc/xray/quota/http", user))
		var quota string
		...
		// IP limit
		ipLimit := ReadFile(filepath.Join("/etc/xray/limit/ip/xray/http", user))
		if ipLimit == "" {
			ipLimit = "Not available"
		}

		// Protocol
		protocolLogPath := fmt.Sprintf("/var/log/create/xray/http/%s.log", user)
