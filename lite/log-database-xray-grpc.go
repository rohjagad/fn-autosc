...
func main() {
	// Mendapatkan daftar akun X-Ray gRPC yang aktif
	cmd := exec.Command("bash", "-c", "ls /var/log/create/xray/grpc | grep -v '\\.locked$' | sed 's/\\.log$//' | sort | uniq")
	out, err := cmd.Output()
	if err != nil {
	...
	clearScreen()
	fmt.Println("===================================")
	fmt.Println("[ Log Database X-Ray gRPC Account ]")
	fmt.Println("===================================")
	...
	logFile := fmt.Sprintf("/var/log/create/xray/grpc/%s.log", username)
