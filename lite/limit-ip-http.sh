...
# Database
username=$(grep '^###' /etc/xray/json/upgrade.json | cut -d ' ' -f 2 | sort | uniq)
...
if ! xray api statsonline --server=127.0.0.1:10081 -email probe 2>&1 | grep -q "not found"; then
    echo "IP limit check skipped: online statistics unavailable on 127.0.0.1:10081"
    exit 0
fi

for user in $username; do
    # Get the limit and current online stats for each user
    limit=$(grep "Limit IP:" /var/log/create/xray/http/${user}.log | awk '{print $3}')
    # Bug 68 guard: 0 / missing / malformed limit = unlimited, never enforced
    if ! [[ "$limit" =~ ^[1-9][0-9]*$ ]]; then
        continue
    fi
    cek=$(xray api statsonline --server=127.0.0.1:10081 -email "$user" 2>/dev/null | jq -r '.stat.value // empty' 2>/dev/null)
